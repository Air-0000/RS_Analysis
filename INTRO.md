...
卫星和无人机多了，高分辨率遥感图怎么自动分析就成了问题。这个项目做两个事：
...
1. **轻量**：两个模型加一起 ~2.6M 参数，普通显卡就能跑
2. **不用预训练**：Tiny U-Net 不用 ImageNet 权重，从零训到 SOTA 的 90%+
3. **开箱即用**：Streamlit 界面，上传图片就出结果
4. **好复现**：一键装环境 + 完整训练脚本
