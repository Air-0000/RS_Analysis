...
采用孪生网络架构，T1 和 T2 图像通过\textbf{共享权重}的编码器提取特征，在特征空间做差（L1 距离）后解码重建变化图。原始参数量 625K。

\textbf{增强版改进（enhanced-model-v1 分支）：}
\begin{itemize}
    \item \textbf{SCConv}：空间-通道重建卷积，替换标准卷积，减少冗余特征
    \item \textbf{CoordAtt}：坐标注意力，编码位置信息增强空间定位
    \item \textbf{CARAFE}：内容感知上采样，替代双线性插值
    \item \textbf{Focal Loss}（$\gamma=2.0,\ \alpha=0.25$）：降低 easy negative 权重，聚焦难例
\end{itemize}
增强版参数量 1.26M，在 LEVIR-CD 上 Val F1 从 \textbf{0.651 提升至 0.723}（+7.1\%），IoU 从 0.477 提升至 0.536（+5.9\%）。
