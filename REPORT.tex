...
成员 & 职责 & 具体工作内容 \\
\midrule
刘正阳 & 方案设计与算法实现 & 模型架构设计、训练脚本编写、超参数调优 \\
李正杨 & 数据处理与界面开发 & 数据集加载模块、Streamlit 交互界面、可视化 \\
龙耀辉 & 报告撰写与测试验证 & 实验记录、性能测试、报告与视频录制 \\
李婉瑶 & 方案复现与错误测试 & 模型复现验证、边界情况测试、交叉验证实验 \\
\bottomrule
\end{tabular}
\end{table}
...
模型层
    ...
    - FC-Siam-diff+ (1.26M) 双时相变化检测（增强版）  输入: T1+T2 256x256x3
        |
数据层
...
模块 & 功能 & 技术栈 & 负责人 \\
\midrule
数据加载模块 & LEVIR-CD / WHU 数据读取 & Python, OpenCV & 李正杨 \\
数据增强模块 & 实时增强（翻转/旋转/归一化） & Albumentations & 刘正阳 \\
模型定义模块 & Tiny U-Net / FC-Siam-diff & PyTorch & 刘正阳 \\
训练模块 & 训练/验证循环、指标记录 & PyTorch, TensorBoard & 刘正阳 \\
推理模块 & 模型加载、前向传播、后处理 & PyTorch, NumPy & 刘正阳 \\
界面模块 & 文件上传、结果展示、交互控制 & Streamlit & 李正杨 \\
测试模块 & 功能/性能/稳定性测试 & Python, time & 龙耀辉、李婉瑶 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{FC-Siam-diff+（增强变化检测）：}

以 FC-Siam-diff 为基础，引入四项改进：

\begin{itemize}
    \item \textbf{SCConv}（Spatial and Channel Reconstruction Convolution）：将标准卷积替换为深度可分离卷积 + 空间-通道门控重建，降低特征冗余，提升表示效率；
    \item \textbf{CoordAtt}（Coordinate Attention）：在通道注意力中嵌入坐标位置信息，使模型能更精准地定位变化区域的空间位置；
    \item \textbf{CARAFE}（Content-Aware ReAssembly of Features）：用内容感知的上采样替代双线性插值，在上采样过程中利用特征内容动态生成重组核，保留更精细的边界信息；
    \item \textbf{Focal Loss}（$\gamma=2.0,\ \alpha=0.25$）：针对 LEVIR-CD 中变化像素仅占 8-15\% 的严重类别不平衡，降低易分负样本的损失权重，迫使模型聚焦于难分的变化区域。
\end{itemize}

改进后模型参数量从 625K 增至 1.26M，在 LEVIR-CD 验证集上 F1 从 \textbf{0.651 提升至 0.723}（+7.1\%），IoU 从 0.477 提升至 0.536（+5.9\%），追平原版 FC-Siam-diff（1M 参数，F1$\sim$0.73）的论文水平。

核心代码：

\begin{lstlisting}
class EnhancedSiamDiff(nn.Module):
    """增强版 FC-Siam-diff — 引入 SCConv + CoordAtt + CARAFE"""
    def forward(self, img_A, img_B):
        def encode(x):
            # 编码器每层用 SCConv 替换标准卷积
            f1 = self.scconv1(x)                     # 256x256x48
            f2 = self.scconv2(self.pool1(f1))         # 128x128x96
            f3 = self.scconv3(self.pool2(f2))         # 64x64x192
            # 每层输出后接 CoordAtt
            f1 = self.ca1(f1); f2 = self.ca2(f2); f3 = self.ca3(f3)
            return f1, f2, f3
        a1, a2, a3 = encode(img_A)
        b1, b2, b3 = encode(img_B)
        # 特征差 (L1) + CARAFE 上采样 + Skip Connection
        d = torch.abs(a3 - b3)
        d = self.carafe(d)                             # CARAFE 上采样
        d = self.dec2(torch.cat([d, torch.abs(a2 - b2)], 1))
        d = self.carafe2(d)
        d = self.dec1(torch.cat([d, torch.abs(a1 - b1)], 1))
        return self.out(d)
\end{lstlisting}
...
Batch Size & 64 & 32 \\
Epochs & 50 & 100 \\
损失函数 & BCEWithLogitsLoss & Focal Loss ($\gamma$=2.0, $\alpha$=0.25) \\\
...
\begin{table}[H]
\centering
\caption{模型性能指标（变化检测为增强版：SCConv+CoordAtt+CARAFE+FocalLoss）}
\label{tab:perf}
\begin{tabular}{lcc}
\toprule
指标 & 语义分割 & 变化检测 \\
...
F1 分数 & 0.879 & 0.723 \\
IoU / mIoU & \textbf{0.877} & \textbf{0.536} \\
\bottomrule
\end{tabular}
\end{table}
